...
public class ImportCore {
	private static final DULocalizator LOC = new DULocalizator(ImportCore.class);
	...
	public static void importRows(final MainView parent, final File importFile) {
		...
		try {
			...
			try {
				DUMessage.showInfoDialog(parent, ImportCore.LOC.getRes("infImportStarted"));

				try (final Workbook workbook = WorkbookFactory.create(importFile)) {
					if (workbook.getNumberOfSheets() < 1) {
						DUMessage.showErrDialog(parent, ImportCore.LOC.getRes("errNoSheetsFound"));
						return;
					}
				...
				DUMessage.showErrDialog(parent, ImportCore.LOC.getRes("errFileNotFoundException"));
				e.printStackTrace();
			} catch (final IOException e) {
				DUMessage.showErrDialog(parent, ImportCore.LOC.getRes("errIOException"));
				e.printStackTrace();
			}
		} finally {
